                ...
                <div class="container-fluid">
                    ...
                    <div class="row mt-5" style="margin-top: 35px; display: flex;">
                        <div class="col-6"
                            ...
                            <b>{{ $orders->first_name . ', ' . $orders->last_name }}</b>
                            <br>
                            <span>
                                <a style="color: white !important;" href="mailto:{{ $orders->email }}" target="_blank">{{ $orders->email }}</a>
                            </span>
                            <br>
                            <span>{{ $orders->phone }}</span>
                        </div>
                        <div class="col-6" style="flex: 0 0 50%; width: 50%; box-sizing: border-box;">
                            <b>Order date:</b> {{ date('d/m/yy H:i', strtotime($orders->created_at)) }}
                            <br>
            ...
            <div class="row mt-2 p-4" style="background-color: white; margin-top: 15px; padding: 20px;">
                <table>
                    <tr>
                        ...
                        @if($orders->payment_type == "pay_later")
                        ...
                        @if($orders->payment_type == "online_payment")
                            <td class="pl-3" style=" padding-left:15px;">
                                <span class="d-inline"
                                      style="color:#424853; font-family:trebuchet,sans-serif; font-size:16px; font-weight:normal; line-height:22px;">
                            ...
                            @foreach($orders->order_details as $orderDetail)
                                <tr>
                                    <td style="border-top: 1px solid #dee2e6; padding: 5px 0;">
                                        {{ $orderDetail->product->product_name . ' (x' . $orderDetail->amount . ')'}}
                                    </td>
